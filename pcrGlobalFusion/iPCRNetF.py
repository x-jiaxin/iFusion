...
from pcrGlobalFusion.fusion import fusion
...
class iPCRNetF(nn.Module):
    def __init__(self, feature_model=PointNet(), droput=0.0, pooling='max'):
        ...
        self.linear = [nn.Linear(1024, 1024),nn.ReLU(),
    ...
    def spam(self, template_feature, source, est_R, est_t):
        ...
        # y = torch.cat([template_feature, source_features], dim=1)

        template_features = template_feature.unsqueeze(1)
        source_features = self.pooling(self.feature_model(source)).unsqueeze(1)
        ...
        y = template_fusion.squeeze(1) + source_fusion.squeeze(1)
