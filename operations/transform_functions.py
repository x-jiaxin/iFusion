...
class PCRNetTransform:
    ...
    def create_pose_6d(vector: torch.Tensor):
        B, _ = vector.size()
        device = vector.device
        one = torch.ones(B, 1).to(device)
        zero = torch.zeros(B, 1).to(device)
        # 3 rotation.
        x = vector[:, 0].reshape(B, 1)
        y = vector[:, 1].reshape(B, 1)
        z = vector[:, 2].reshape(B, 1)

        # B x 6 vector of 3 quaternions and 3 translation parameters
        t = vector[:, 3:].reshape(B, 1, 3)

        xmat = torch.stack([one, zero, zero,
                            zero, torch.cos(x), torch.sin(x) * (-1),
                            zero, torch.sin(x), torch.cos(x)], dim=1).reshape(B, 3, 3)
        ymat = torch.stack([torch.cos(y), zero, torch.sin(y),
                            zero, one, zero,
                            torch.sin(y) * (-1), zero, torch.cos(y)], dim=1).reshape(B, 3, 3)
        zmat = torch.stack([torch.cos(z), torch.sin(z) * (-1), zero,
                            torch.sin(z), torch.cos(z), zero,
                            zero, zero, one], dim=1).reshape(B, 3, 3)
        R = (xmat * ymat) * zmat
        return R, t
    ...
    def quaternion_transform2(point_cloud: torch.Tensor, pose_8d: torch.Tensor, T: torch.Tensor):
        transformed_point_cloud = PCRNetTransform.quaternion_rotate(point_cloud, pose_8d) + T.view(-1, 1, 3).repeat(1,
                                                                                                                    point_cloud.shape[
                                                                                                                        1],
                                                                                                                    1)  # Ps' = R*Ps + t
        return transformed_point_cloud
