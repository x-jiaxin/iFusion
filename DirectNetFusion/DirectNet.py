...
from DirectNetFusion.Pointnet import PointNet
...
class Directnet(nn.Module):
    ...
    def forward(self, template, source):
        ...
        est_R, est_t, = self.spam(template, source, est_R, est_t)
    ...
    def spam(self, template, source, est_R, est_t):
        sourceFeatures = self.pooling(self.feature_model(source))  # 源点云的全局特征
        templateFeatures = self.pooling(self.feature_model((template)))  # 源点云的全局特征
