...
def train(model, train_loader, test_loader):
    ...
    for epoch in range(START_EPOCH, MAX_EPOCHS):
        ...
        if test_loss < best_test_loss:
            ...
            torch.save(
                snap, "../checkpoints/{}/models/best_model_snap.t7".format(exp_name)
            )
            torch.save(
                model.module.state_dict(),
                "../checkpoints/{}/models/best_model.t7".format(exp_name),
            )
            torch.save(
                model.module.feature_model.state_dict(),
                "../checkpoints/{}/models/best_ptnet_model.t7".format(exp_name),
            )

        torch.save(snap, "../checkpoints/{}/models/model_snap.t7".format(exp_name))
        torch.save(
            model.module.state_dict(),
            "../checkpoints/{}/models/model.t7".format(exp_name),
        )
        torch.save(
            model.module.feature_model.state_dict(),
            "../checkpoints/{}/models/ptnet_model.t7".format(exp_name),
        )

        print(
            "EPOCH:{},Training Loss:{},Testing Loss:{},Best Loss:{}".format(
